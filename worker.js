...
// Import all your processors
import { fetchAndProcessOrders } from './processor.js'; // Assuming this is Eileen Grace
...
workerApp.listen(port, () => { console.log("Worker listening on port: ", port); });

// ACTION: Define the Worker Options with Rate Limiting
const workerOptions = {
    connection: {
    ...
    // THE MAGIC FIX:
    limiter: {
        max: 1,      // Process max 1 job...
        duration: 10000 // ...every 10 seconds.
    }
};

// ACTION: One Central Processor Function
const mainBrandProcessor = async (job) => {
...
console.log("Single Brand Worker is starting!");

// ACTION: Initialize ONLY ONE Worker
const brandWorker = new Worker("brand-sync-queue", mainBrandProcessor, workerOptions);

// Global Event Listeners
brandWorker.on('active', (job) => {
    console.log(`[${job.name}] ACTIVE. (ID: ${job.id})`);
});
brandWorker.on('completed', (job) => {
    console.log(`[${job.name}] COMPLETED.`);
});
brandWorker.on('failed', (job, err) => {
    console.error(`[${job.name}] FAILED. Error:`, err);
});
brandWorker.on('error', (err) => {
    console.error('[Worker] Redis Connection Error:', err);
});

const gracefulShutdown = async () => {
    console.log("Shutting down worker...");
    await brandWorker.close();
