...
async function fetchWalletTransaction(brand, partner_id, partner_key, access_token, shop_id) {    
    ...
    try {
        ...
        while(hasMore) {
            ...
            const createTimeFrom = Math.floor(new Date(Date.now() - 86400000).setHours(0, 0, 0, 0) / 1000);
            const createTimeTo = Math.floor(new Date(Date.now() - 86400000).setHours(23, 59, 59, 999) / 1000);
