...
export async function fetchLGMVMaxBreakdown(brand, advertiser_id) {
    ...
    try {
        ...
        if(response && response.data && response.data.data && response.data.data.list) {
            ...
            console.log(`[LIVE-BREAKDOWN] response does not exist on ${brand}`);
        }
                ...
                while(!success && retries > 0) {
                    try {
                        const params = {
                            ...
                            metrics: JSON.stringify(["cost", "gross_revenue"]),
                        ...
                        console.log(`🔥 [LIVE-BREAKDOWN] raw response product-level metrics: `);
                        const nonZeroList = response.data.data.list.filter(item => parseInt(item.metrics.cost) !== 0 || parseInt(item.metrics.gross_revenue) !== 0);
                        ...
                        //     if(brand == "SHRD") {
                        //         console.log(`🔥 [LIVE-BREAKDOWN] product-level metrics response on ${brand} and Campaign Name: ${c.campaign_name} and ID: ${c.campaign_id}`);
                        //         // console.log(response.data.data.list);
                        //     }
                        ...
                        // } else {
                        //     retries -= 1;
                        //     console.log(`🥶 [LIVE-BREAKDOWN] Failed response on ${brand}. Retries left: ${retries}`);
                        //     // console.log(response?.data);

                        //     if(retries > 0) await sleep(25000);
                        // }
                    } catch (e) {
                        console.log("🤯 [LIVE-BREAKDOWN] Error getting product-level metrics on: ", brand, "error: ", e);
                    }
                }
            }

            if(breakdownCostList.length > 0) {
                console.log(`🔥 [LIVE-BREAKDOWN] Breakdown Cost List on ${brand}`);
        ...
        console.log("🤯 [LIVE-BREAKDOWN] Error getting campaign_id on: ", brand, "error: ", e);
    }
}
...
async function preprocessData(advertiser_id, brandName, tableName, dataReference, campaignIdList) {
    ...
    // console.log("Campaign Metrics List: ");
    // console.log(campaignMetricsList);
    ...
    console.log("Processed data to merge: ");
...
let brand = "Mosseru";
let advertiser_id = "7553574194160746513"
await fetchLGMVMaxBreakdown(brand, advertiser_id)