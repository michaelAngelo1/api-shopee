...
export async function handleOrdersMD(orderDetails, orderEscrows) {
    ...
    orderDetails.forEach(o => {
        ...
        if(o.item_list.length > 1) {
            for(let i=0; i<o.item_list.length; i++) {
                ...
                MDOrdersObjectList.push(sampleOrderObject);
            }
        } else {
    ...
    if(MDOrdersObjectList && MDOrdersObjectList.length > 0) {
        console.log("MD: Passing MDOrdersObjectList to mergeOrders \n");
        console.log(MDOrdersObjectList.length + " order rows to be merged");

        await mergeOrdersMD(MDOrdersObjectList);
    } else {
        console.log("MD: No orders were eligible for merging. sampleOrderObjectList is empty. Skipping BigQuery.");
    }
}

async function mergeOrdersMD(orders) {
    console.log("MD Orders to Merge");

    try {
        const datasetId = 'shopee_api';
        const tableNameStaging = 'miss_daisy_orders_staging';
        ...
        console.log("MD: Successfully written orders to miss_daisy_orders_staging");

        const mergeQuery = `
            MERGE \`shopee_api.miss_daisy_orders\` T
            USING \`shopee_api.miss_daisy_orders_staging\` S
        ...
        await bigquery.query({ query: `TRUNCATE TABLE \`shopee_api.miss_daisy_orders_staging\`` });
        console.log(`Inserted ${orders.length} rows to miss_daisy_orders`);
    } catch (e) {
        console.error("MD: An error occurred during the BigQuery operation.");

        if (e.name === 'PartialFailureError' && e.errors && e.errors.length > 0) {
            console.error("MD: Some rows failed to insert into the staging table. See details below:");
            ...
            console.error("MD: A non-partial or unknown error occurred:", e);
        }
    }
}