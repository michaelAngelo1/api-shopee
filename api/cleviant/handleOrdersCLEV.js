...
export async function handleOrdersCLEV(orderDetails, orderEscrows) {
    ...
    orderDetails.forEach(o => {
        ...
        if(o.item_list.length > 1) {
            for(let i=0; i<o.item_list.length; i++) {
                ...
                CLEVOrdersObjectList.push(sampleOrderObject);
            }
        } else {
    ...
    if(CLEVOrdersObjectList && CLEVOrdersObjectList.length > 0) {
        console.log("CLEV: Passing CLEVOrdersObjectList to mergeOrders \n");
        console.log(CLEVOrdersObjectList.length + " order rows to be merged");

        await mergeOrdersCLEV(CLEVOrdersObjectList);
    } else {
        console.log("CLEV: No orders were eligible for merging. sampleOrderObjectList is empty. Skipping BigQuery.");
    }
}

async function mergeOrdersCLEV(orders) {
    console.log("CLEV Orders to Merge");

    try {
        const datasetId = 'shopee_api';
        const tableNameStaging = 'cleviant_orders_staging';
        ...
        console.log("CLEV: Successfully written orders to cleviant_orders_staging");

        const mergeQuery = `
            MERGE \`shopee_api.cleviant_orders\` T
            USING \`shopee_api.cleviant_orders_staging\` S
        ...
        await bigquery.query({ query: `TRUNCATE TABLE \`shopee_api.cleviant_orders_staging\`` });
        console.log(`Inserted ${orders.length} rows to cleviant_orders`);
    } catch (e) {
        console.error("CLEV: An error occurred during the BigQuery operation.");

        if (e.name === 'PartialFailureError' && e.errors && e.errors.length > 0) {
            console.error("CLEV: Some rows failed to insert into the staging table. See details below:");
            ...
            console.error("CLEV: A non-partial or unknown error occurred:", e);
        }
    }
}