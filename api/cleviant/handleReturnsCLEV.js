...
export async function handleReturnsCLEV(orderReturns) {
    console.log("CLEV: Merging order returns Cleviant");
    ...
    console.log(`CLEV: Received ${orderReturns.length} raw returns, de-duplicated to ${uniqueLatestReturns.length} unique latest returns.`);

    const [rows] = await bigquery.query({
        query: `
            SELECT No_Pesanan, Return_Status
            FROM \`shopee_api.cleviant_return_refund\`
    ...
    const tableIdStaging = 'cleviant_return_refund_staging';

    if (orderReturnsToWrite.length === 0) {
        console.log("CLEV: No new or updated returns to process.");
    ...
    try {
        ...
        console.log(`CLEV: Inserted ${orderReturnsToWrite.length} cleviant_return_refund_staging`);

        const mergeQuery = `
            MERGE \`shopee_api.cleviant_return_refund\` T
            USING (
              SELECT * FROM \`shopee_api.cleviant_return_refund_staging\`
              QUALIFY ROW_NUMBER() OVER(PARTITION BY No_Pesanan ORDER BY Update_Time DESC) = 1
            ) S
        ...
        await bigquery.query({ query: `TRUNCATE TABLE \`shopee_api.cleviant_return_refund_staging\``});
        console.log(`CLEV HandleReturns: Inserted ${orderReturnsToWrite.length} rows to cleviant_return_refund`);
    } catch (e) {
        console.error("CLEV HandleReturns: An error occurred during the BigQuery operation.");

        if (e.name === 'PartialFailureError' && e.errors && e.errors.length > 0) {
            console.error("CLEV HandleReturns: Some rows failed to insert into the staging table. See details below:");
            ...
            console.error("CLEV HandleReturns: A non-partial or unknown error occurred:", e);
        }
    }
}