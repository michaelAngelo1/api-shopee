...
export async function getOrderDetailCLEV(orderList) {
    ...
    try {
        ...
        for(const orderIdChunk of orderIdChunks) {
            ...
            console.log("CLEV: Hitting Order Detail endpoint: ", fullUrl);
            ...
            if(response && response.data.response && Array.isArray(response.data.response.order_list)) {
                CLEVOrdersWithDetail = CLEVOrdersWithDetail.concat(response.data.response.order_list);
                console.log("CLEV: order detail exists");
            }
        }

        return CLEVOrdersWithDetail
    } catch (e) {
        console.log("CLEV: Error getting order detail: ", e);
    }
}