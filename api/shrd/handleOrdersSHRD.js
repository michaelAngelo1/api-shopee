...
export async function handleOrdersSHRD(orderDetails, orderEscrows) {
    ...
    orderDetails.forEach(o => {
        ...
        if(o.item_list.length > 1) {
            for(let i=0; i<o.item_list.length; i++) {
                ...
                SHRDOrdersObjectList.push(sampleOrderObject);
            }
        } else {
    ...
    if(SHRDOrdersObjectList && SHRDOrdersObjectList.length > 0) {
        console.log("SHRD: Passing SHRDOrdersObjectList to mergeOrders \n");
        console.log(SHRDOrdersObjectList.length + " order rows to be merged");

        await mergeOrdersSHRD(SHRDOrdersObjectList);
    } else {
        console.log("SHRD: No orders were eligible for merging. sampleOrderObjectList is empty. Skipping BigQuery.");
    }
}

async function mergeOrdersSHRD(orders) {
    console.log("SHRD Orders to Merge");

    try {
        const datasetId = 'shopee_api';
        const tableNameStaging = 'shrd_orders_staging';
        ...
        console.log("SHRD: Successfully written orders to shrd_orders_staging");

        const mergeQuery = `
            MERGE \`shopee_api.shrd_orders\` T
            USING \`shopee_api.shrd_orders_staging\` S
        ...
        await bigquery.query({ query: `TRUNCATE TABLE \`shopee_api.shrd_orders_staging\`` });
        console.log(`Inserted ${orders.length} rows to shrd_orders`);
    } catch (e) {
        console.error("SHRD: An error occurred during the BigQuery operation.");

        if (e.name === 'PartialFailureError' && e.errors && e.errors.length > 0) {
            console.error("SHRD: Some rows failed to insert into the staging table. See details below:");
            ...
            console.error("SHRD: A non-partial or unknown error occurred:", e);
        }
    }
}